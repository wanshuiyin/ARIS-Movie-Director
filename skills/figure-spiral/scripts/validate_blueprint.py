#!/usr/bin/env python3
"""validate_blueprint.py — structural validation of a figure-spiral blueprint (pure stdlib).
...
def main():
    ...
    for n in nodes:
        ...
        p = n.get("pos") or {}
        if cw and ch and "x" in p and "y" in p and not (0 <= p["x"] <= cw and 0 <= p["y"] <= ch):
            errs.append(f"node '{n.get('id')}' pos {p} outside canvas {cw}x{ch}")
